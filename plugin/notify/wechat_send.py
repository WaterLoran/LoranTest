...
from plugin.exceptions import SendMessageError, ValueTypeError
...
class WeChatSend:
        ...
        if mentioned_mobile_list is None or isinstance(mentioned_mobile_list, list):
            ...
            if len(mentioned_mobile_list) >= 1:
                for i in mentioned_mobile_list:
                    if isinstance(i, str):
                        res = requests.post(url=wechat_webhook, json=_data, headers=self.headers)
    ...
    def send_wechat_notification(self):
        ...
        text = f"""【{project_name}自动化通知】
                                    ...
                                    >时间：<font color=\"comment\">{DateTimeTool().getNowTime()}</font>
                                    >
                                    >非相关负责人员可忽略此消息。
...
if __name__ == '__main__':
    ...
    # TODO 未在config.notify中设置wechat的相关机器人的webhook的信息,此脚本还未能用